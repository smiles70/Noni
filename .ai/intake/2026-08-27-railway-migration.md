...
Fly.io account has an overdue-invoice block, preventing `flyctl deploy` from completing. The production frontend build (Cloudflare Pages) is also currently coupled to the `noni-api.fly.dev` backend URL. A migration to Railway, which the project owner already uses for other apps, removes the billing blocker and keeps Cloudflare Pages for the frontend.

## Scope

1. Replace Fly.io backend deploy with Railway.
2. Keep Cloudflare Pages for the frontend.
3. Migrate secrets from `flyctl secrets set` to `railway variables set`.
...
- The backend URL will change from `https://noni-api.fly.dev` to a Railway-provided URL (or custom domain).
