...
- **Performance / smoke gates:** bundle-size is verified, but no k6 / Lighthouse / pa11y smoke is wired to CI, and the backend A10 full-launch smoke has not been run in isolation against a clean Postgres instance.
...
| Backend full-launch smoke | `backend/tests/test_a10_smoke.py` with Postgres | All 7 A10 scenarios green |
| API latency baseline | `k6` or `ab` | p95 < 200 ms for public GETs, p95 < 500 ms for mutations |
| Lighthouse performance | `lighthouse` or `playwright-lighthouse` | Performance ≥ 90 on landing, paywall, gift |
| Pa11y regression | `pa11y` | 0 WCAG 2.1 AA errors on `/`, `/gift`, `/signin`, `/paywall` |

---

## 3. Acceptance criteria

- [ ] `npm run test:e2e` is green across chromium, firefox, webkit, mobile-pixel, mobile-iphone in CI.
- [ ] New E2E specs cover onboarding, paywall, admin, account, partner/community, and auth callback.
- [ ] `backend/tests/test_a10_smoke.py` passes against a clean Postgres instance with `DATABASE_URL` set.
- [ ] Bundle-size check remains green after every build.
- [ ] k6 or equivalent smoke records p95 latency for public endpoints.
- [ ] Lighthouse / pa11y reports are generated and committed to `.ai/research/` or linked in the preflight.
- [ ] PREFLIGHT_TESTING_MATURITY_001 updated to mark Blocks 4 and 5 COMPLETE.
