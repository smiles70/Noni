...
- **Integrations:** Mock auth (dev/tests); **Magic.link integrated** as production auth provider (ADR 0027). Mock remains the default for CI/local; Magic is opt-in via `AUTH_PROVIDER=magic` + `MAGIC_API_SECRET_KEY`. Stripe (payments), Fly.io (backend), Cloudflare Pages (frontend).
...
## Production-readiness and hygiene scores

- PRA: 91 / 100 — Enterprise Mature — Conditional on Magic live-key deploy and runtime infra
- Nelson: 88 / 100 — Enterprise Mature

## Blockers

No code-level production blockers remain. The open items are runtime/infrastructure provisioning.
