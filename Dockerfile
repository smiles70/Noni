...
CMD ["sh", "-c", "if [ \"$SERVICE_ROLE\" = \"worker\" ]; then celery -A backend.tasks.celery_app worker -B -s /tmp/celerybeat-schedule -l INFO -Q realtime,events,batch,ai; else alembic upgrade head && gunicorn -k uvicorn.workers.UvicornWorker -w ${WEB_CONCURRENCY:-3} --bind 0.0.0.0:${PORT} backend.app.main:app; fi"]
