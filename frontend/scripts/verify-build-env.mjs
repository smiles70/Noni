 ...
 * 
 * Sprint: G3 Prevention — Fail-fast guard for production builds
...
if (!isProduction && !isCI) {
  console.log('✓ Build env verification: SKIPPED (dev mode)');
  exit(0);
}
...
if (!apiBaseUrl) {
  console.error('\n❌ BUILD FAILED: VITE_API_BASE_URL is not set');
...
if (apiBaseUrl.includes('localhost') || apiBaseUrl.includes('127.0.0.1')) {
  console.error('\n❌ BUILD FAILED: VITE_API_BASE_URL contains localhost');
...
if (!apiBaseUrl.startsWith('https://') && !apiBaseUrl.includes('ngrok')) {
  console.error('\n❌ BUILD FAILED: VITE_API_BASE_URL must use HTTPS in production');
  console.error(`   Current value: ${apiBaseUrl}`);
