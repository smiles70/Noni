"""CI contract: GET /api/v1/auth/session with a malformed Bearer token must NOT 500.

A 500 on the auth session endpoint caused the frontend's auth bootstrap to
retry forever, producing the infinite redirect loop observed during the
auth sprint. The correct behavior is a clean 401. Anything in the 5xx
...
def main() -> int:
    ...
    if 500 <= r.status_code < 600:
        print(
            f"\u274c auth session returned HTTP {r.status_code} on a malformed "
            f"Bearer token; must be 4xx. Body: {r.text[:200]!r}"
        )
        return 1

    print(
        f"\u2705 auth session stability OK "
