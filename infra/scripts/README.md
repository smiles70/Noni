...
| `secrets-sync.sh` | `make secrets-sync` | Push every secret to Railway, Cloudflare Pages, and GitHub Actions. |
...
| `deploy-prod.sh` | `make deploy-prod` | Supabase migrations + Railway deploy + Cloudflare Pages deploy + smoke. |
...
- All scripts require the relevant vendor CLI (`railway`, `wrangler`, `supabase`, `stripe`, `sops`, etc.) to be authenticated.
