...
#
# Full production deploy: Supabase migrations, Railway backend (with Alembic via
...
# When the database is Railway Postgres (or any non-Supabase Postgres) the
...
echo "==> 2/4 railway deploy (Alembic runs in release_command)"
railway deploy --remote-only --app "$RAILWAY_APP" || { echo "ERROR: railway deploy failed"; exit 1; }
